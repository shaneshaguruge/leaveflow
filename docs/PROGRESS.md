...
- [x] The Jest unit suite passes, including the (fixed) Vesak poya holiday case — `cd server && npx jest --runInBand --json` → 32 passed, 0 failed (2026-09-21, after #27/#28), incl. "leaveDays excludes Vesak poya from a spanning request"; CI test-api green on `main` @ `7ae5945`: https://github.com/shaneshaguruge/leaveflow/actions/runs/35587300869
...
- [x] The ApplyLeaveForm Vitest test passes with vitest run — `cd client && npx vitest run --reporter=verbose` → 8 passed (5 in `ApplyLeaveForm.test.jsx`); CI test-client green: run 35587300869
- [x] The Playwright apply-approve spec passes with webServer booting both apps — `npx playwright test` on `main` → 3 passed (8.8s in the 2026-09-21 audit; earlier 7.7s on `e0f3abf`), incl. "employee applies, manager approves, employee sees APPROVED and her balance change"; not in CI
